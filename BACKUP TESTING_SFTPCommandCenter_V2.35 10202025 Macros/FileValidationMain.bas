...
Private Function ValidateSelectedFile(sFilePath As String) As ValidationResult
    ...
    DebugCheckMappingKeysCM oMapping, sFilePath
...
' Validates that every expected field in ColumnMapping has a positive 1-based column index
Private Sub DebugCheckMappingKeysCM(ByRef m As ColumnMapping, ByVal srcFile As String)
    Dim missing As String
    ' Adjust these member names only if your ColumnMapping uses different property names
    If m.FirstName <= 0 Then missing = missing & "FirstName,"
    If m.LastName <= 0 Then missing = missing & "LastName,"
    If m.DOB <= 0 Then missing = missing & "DOB,"
    If m.Gender <= 0 Then missing = missing & "Gender,"
    If m.ZipCode <= 0 Then missing = missing & "ZipCode,"
    If m.Address1 <= 0 Then missing = missing & "Address1,"
    If m.Address2 <= 0 Then missing = missing & "Address2,"        ' <-- this is the common culprit
    If m.City <= 0 Then missing = missing & "City,"
    If m.State <= 0 Then missing = missing & "State,"
    If m.EffectiveDate <= 0 Then missing = missing & "EffectiveDate,"
    If m.groupID <= 0 Then missing = missing & "GroupID,"
    If m.serviceOffering <= 0 Then missing = missing & "ServiceOffering,"
    If m.memberID <= 0 Then missing = missing & "MemberID,"
    ' Optional: If you require a column for EffectiveEndDate, change < 0 to <= 0
    If m.effectiveEndDate < 0 Then missing = missing & "EffectiveEndDate,"

    If Len(missing) > 0 Then
        missing = Left$(missing, Len(missing) - 1) ' trim trailing comma
        Err.Raise vbObjectError + 700, "ValidateSelectedFile", _
                  "Missing/zero mapping for: " & missing & _
                  " (FileType " & m.fileType & ")." & vbCrLf & _
                  "Check 'Filetype Mapping' for this type." & vbCrLf & _
                  "File: " & srcFile
    End If
End Sub
    ...
    For lRow = 2 To lTotalRows
        ...
        If oFileInfo.groupID <> "MANUAL" Then
            If oMapping.groupID > 0 And oMapping.groupID <= UBound(vData, 2) Then
                ...
                End If
                              ' After all field validations, check for duplicates
                              
            Call CheckForDuplicates_FIXED(vData, oMapping, oResult, 2, lTotalRows)
...
Private Sub ValidateRowFields(vData As Variant, lRow As Long, oMapping As ColumnMapping, colRules As Collection, oResult As ValidationResult)
    ...
    If oMapping.FirstName > 0 And oMapping.FirstName <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.FirstName), "FirstName", lRow, colRules, oResult)
    End If
    
    ' Validate Last Name
    If oMapping.LastName > 0 And oMapping.LastName <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.LastName), "LastName", lRow, colRules, oResult)
    End If
    
    ' Validate DOB
    If oMapping.DOB > 0 And oMapping.DOB <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.DOB), "DOB", lRow, colRules, oResult)
    End If
    
    ' Validate Gender
    If oMapping.Gender > 0 And oMapping.Gender <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.Gender), "Gender", lRow, colRules, oResult)
    End If
    
    ' Validate Zip Code
    If oMapping.ZipCode > 0 And oMapping.ZipCode <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.ZipCode), "ZipCode", lRow, colRules, oResult)
    End If
    
    ' Validate Address1
    If oMapping.Address1 > 0 And oMapping.Address1 <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.Address1), "Address1", lRow, colRules, oResult)
    End If
    
    ' Validate City
    If oMapping.City > 0 And oMapping.City <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.City), "City", lRow, colRules, oResult)
    End If
    
    ' Validate State
    If oMapping.State > 0 And oMapping.State <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.State), "State", lRow, colRules, oResult)
    End If
    
    ' Validate Effective Date
    If oMapping.EffectiveDate > 0 And oMapping.EffectiveDate <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.EffectiveDate), "EffectiveDate", lRow, colRules, oResult)
    End If
    
    ' Validate Service Offering
    If oMapping.serviceOffering > 0 And oMapping.serviceOffering <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.serviceOffering), "ServiceOffering", lRow, colRules, oResult)
    End If
End Sub
...
Private Function ActiveChecker(sEffectiveEndDate As String) As Boolean
    ...
    ActiveChecker = ActiveChecker_FIXED(sEffectiveEndDate)
End Function
