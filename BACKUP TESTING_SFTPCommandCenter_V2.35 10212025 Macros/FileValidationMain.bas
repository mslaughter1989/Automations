...
Private Sub ValidateCSVDataIntoResult(vData As Variant, oMapping As ColumnMapping, colRules As Collection, oFileInfo As FileInfo, oResult As ValidationResult)
    ...
    For lRow = 2 To lTotalRows
        ...
        ' Validate individual fields for this row
        ...
        If oFileInfo.groupID <> "MANUAL" Then
            ...
            End If
        End If
    Next lRow  ' <-- END OF LOOP
    
    ' *** MOVED OUTSIDE THE LOOP - ONLY RUN ONCE PER FILE! ***
    Debug.Print "Running duplicate check on entire dataset..."
    Call CheckForDuplicates_FIXED(vData, oMapping, oResult, 2, lTotalRows)
    Debug.Print "Duplicate check complete."
...
Private Sub ValidateRowFields(vData As Variant, lRow As Long, oMapping As ColumnMapping, colRules As Collection, oResult As ValidationResult)
    ...
    If oMapping.memberID > 0 And oMapping.memberID <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.memberID), "MemberID", lRow, colRules, oResult)
    End If
    
    ' Validate Group ID
    If oMapping.groupID > 0 And oMapping.groupID <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.groupID), "GroupID", lRow, colRules, oResult)
    End If
    
    ' Validate Effective End Date
    If oMapping.effectiveEndDate > 0 And oMapping.effectiveEndDate <= UBound(vData, 2) Then
        Call VALIDATION_FIX_COMPLETE.ValidateField(vData(lRow, oMapping.effectiveEndDate), "EffectiveEndDate", lRow, colRules, oResult)
    End If
End Sub
...
Private Sub CreateValidationSummaryMatrix(ws As Worksheet, oResult As ValidationResult, lStartRow As Long)
    ...
    ' Set specific column widths instead of AutoFit
ws.Columns("A").ColumnWidth = 1     ' Keep field name column auto-sized
ws.Columns("B").ColumnWidth = 1  ' Narrow width for Blank Check
ws.Columns("C").ColumnWidth = 1   ' Narrow width for Max Character Check
ws.Columns("D").ColumnWidth = 25   ' Narrow width for Format Check
