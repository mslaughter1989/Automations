        ...
        If oFileInfo.GroupID <> "MANUAL" Then
            If oMapping.GroupID > 0 And oMapping.GroupID <= UBound(vData, 2) Then
                ...
                If lRow <= 5 Then
                    ...
                    Debug.Print "Row " & lRow & " - Length: " & Len(sGroupID)
                End If
                
                ' Check if the GroupID from the file is in the list of expected GroupIDs
                Dim bGroupIDValid As Boolean
                bGroupIDValid = False
                
                ' If oFileInfo.GroupID contains multiple IDs separated by underscore
                If InStr(oFileInfo.GroupID, "_") > 0 Then
                    ' Split the expected GroupIDs into an array
                    Dim vExpectedGroupIDs As Variant
                    vExpectedGroupIDs = Split(oFileInfo.GroupID, "_")
                    
                    ' Debug output for first row only
                    If lRow = 2 Then
                        Debug.Print "Split resulted in " & UBound(vExpectedGroupIDs) + 1 & " GroupIDs:"
                        Dim j As Integer
                        For j = 0 To UBound(vExpectedGroupIDs)
                            Debug.Print "  [" & j & "] = '" & vExpectedGroupIDs(j) & "' (Length: " & Len(vExpectedGroupIDs(j)) & ")"
                        Next j
                    End If
                    
                    ' Check if the current GroupID matches any of the expected ones
                    Dim i As Integer
                    For i = 0 To UBound(vExpectedGroupIDs)
                        Dim sExpectedID As String
                        sExpectedID = Trim(CStr(vExpectedGroupIDs(i)))
                        
                        ' Debug comparison for first few rows
                        If lRow <= 5 Then
                            Debug.Print "  Comparing '" & sGroupID & "' with '" & sExpectedID & "'"
                            Debug.Print "    Match? " & (sGroupID = sExpectedID)
                            ' Also check numeric comparison in case of numeric issues
                            If IsNumeric(sGroupID) And IsNumeric(sExpectedID) Then
                                Debug.Print "    Numeric values: " & Val(sGroupID) & " vs " & Val(sExpectedID)
                                Debug.Print "    Numeric match? " & (Val(sGroupID) = Val(sExpectedID))
                            End If
                        End If
                        
                        If sExpectedID = sGroupID Then
                            bGroupIDValid = True
                            If lRow <= 5 Then
                    ...
                    Next i
                Else
                    ' Single GroupID - direct comparison
                    bGroupIDValid = (sGroupID = oFileInfo.GroupID)
                    If lRow <= 5 Then
                        Debug.Print "Single GroupID comparison: '" & sGroupID & "' = '" & oFileInfo.GroupID & "' ? " & bGroupIDValid
                    End If
                End If
